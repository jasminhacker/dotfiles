...
if [ "$TERM" = "xterm-256color" ] && [ -n "$TMUX" ]; then
	# usually this path is executed on tmux with screen TERM
	# however, our tmux's TERM is set to xterm-256color,
	# because screen-256color does not support italics.
	# thus we force the screen color modifications for this case
